...
這份是判準的唯一來源。執行流程（前置條件、閘門、commit、報告）不在這裡，在 `agents/comment-cleanup.md`。
...
1. **定範圍**——當前 branch 相對主幹的所有變更，**包含還沒 commit 的**：`git diff --name-only --diff-filter=d $(git merge-base HEAD <base>)`，base 取該 repo 的主幹分支。注意不是 `<merge-base>..HEAD`——後者只到 HEAD，會漏掉工作區裡的變更。使用者指定路徑就照指定。**不要**擴大到整個 repo 清歷史遺留，除非明確要求。
...
逐檔以精準字串替換只刪／改註解行。**不要跑 formatter 或 lint --fix**（`prettier --write`、`eslint --fix`、`pnpm style:fix` 之類）：那會重排 code、把無關變更混進 diff，直接違反鐵則。
